...
module.exports.tokenHandler = async (tokenRequestBody, tokenRequestHeaders) => {
	...
	var formData = await get_okta_token_request(JSON.parse(tokenRequestBody), tokenRequestHeaders, tokenEndpoint)
...
async function get_private_key_jwt(client_id, token_endpoint) {
	const clientId = client_id;
	const now = Math.floor( new Date().getTime() / 1000 );
	const plus5Minutes = new Date( ( now + (5*60) ) * 1000);
	//var signingKeyPrivate = fs.readFileSync('private_key.pem')
	var signingKeyPrivate= myCache.get( "PrivSign" );

    if ( signingKeyPrivate  == undefined ){
        console.log("Loading PrivSign from Secrets......");  
		try{
			var signingKeyPrivate = await AccessSecret("projects/577362005659/secrets/SMART_PRIVATE/versions/latest", function (err) {
			console.log('Read signingKeyPrivate from storage...');
			});;

			myCache.set("PrivSign", signingKeyPrivate.toString());
			console.log("PrivSign cached ......");
		}
		catch (err) {
			console.log(err);
			return err;
...
async function get_okta_token_request(requestBody, requestHeaders, tokenEndpoint) {
  //3 valid scenarios:
  ...
  //4- **FUTURE** public client w/ PKCE refresh token request
	...
	if(requestBody.grant_type == 'authorization_code' && !confidentialClient) {
		return 'client_assertion=' +
			await get_private_key_jwt(clientId, tokenEndpoint) +
