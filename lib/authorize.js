...
module.exports.authorizeHandler = async (requestQuerystring) => {
	...
	catch(scopeParseError) {
		console.log(scopeParseError)
		return {
			statusCode: 400,
			body: 'Unable to validate parse the scope(s) passed in.',
			location: null,
	...
	audParam = audParam.replace(/\/$/, '');
	const audurl = new URL(audParam);
	const aud = audurl.origin;

	if(!aud || aud !== process.env.EXPECTED_AUD_VALUE) {
